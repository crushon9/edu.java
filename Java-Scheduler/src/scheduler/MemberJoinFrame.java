...
public class MemberJoinFrame extends JFrame {
	...
	public MemberJoinFrame(MemberDAOImple memberDao) {
	...
	private int idCheck(MemberDAOImple memberDao) {
		...
		if (inputId.equals("")) {
			JOptionPane.showMessageDialog(contentPane, "아이디를 입력해주세요");
			return resultId;
		}
		...
		try {
			...
			if (IdExistFlag == 1) {
				JOptionPane.showMessageDialog(contentPane, "이미 가입된 아이디입니다");
				textId.setText("");
			} else {
				JOptionPane.showMessageDialog(contentPane, "가입 가능한 아이디입니다");
				resultId = 1;
			}
		} catch (Exception e) {
			e.getStackTrace();
